...
class CaptioningSolver_dataiter(object):
  """
  ...
  def __init__(self, model, dataiter, **kwargs):
    """
    Construct a new CaptioningSolver instance.
    
    Required arguments:
    - model: A model object conforming to the API described above
    - dataiter: A dictionary of training and validation data from load_coco_data
  ...
  def train(self):
    ...
    """
    num_train = self.dataiter.data['train_captions'].shape[0]
    iterations_per_epoch = max(num_train / self.batch_size, 1)
    num_iterations = self.num_epochs * iterations_per_epoch

    for minibatch in self.dataiter.get_batch():
